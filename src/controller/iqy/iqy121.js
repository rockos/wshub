...
var demoGauge01 = function(){
    ...
    lcsSOCK.io().of('/scr/121').emit("gauge01", {"value": rand});
    setTimeout(demoGauge01,700);
}
...
exports.sockMain = function(){
    lcsSOCK.io().of('/scr/121').on('connection', function(socket) {
        /*connect 正常*/
        console.log( "connect---" + "/scr/121" );
        lcsSOCK.io().of('/scr/121').sockets[socket.id].emit("debugz", "conn::"+socket.id+"<br>");

        /*クライアントからエミットされた*/
        socket.on("massage", function(data) {
            lcsSOCK.io().of('/scr/121').sockets[socket.id].emit("debugz", "massage::"+socket.id+"<br>");
            lcsSOCK.io().of('/scr/121').sockets[socket.id].emit("debugz", data);
        });
