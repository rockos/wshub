   ...
   lcsUI.prototype.checkVal = function(req, params){
       ...
       var errors = {}, errmsg = {};
        for (var i = 0,max = params.length; i < max; i++) {
            ...
            case 'pcode':
            req.check('pcode', '101').len(5,6); /* invalid pcode */
            ...
            case 'pnam':
            req.check('pnam', '102').len(4,4); /* invalid lotn */
            errors = req.validationErrors();
            if (errors) {
                errmsg = lcsAp.getMsgI18N(errors[0].msg);
                return errmsg;
            ...
            case 'lotn':
            req.check('lotn', '103').len(2,5); /* invalid lotn */
            errors = req.validationErrors();
            if (errors) {
                errmsg = lcsAp.getMsgI18N(errors[0].msg);
                return errmsg;
            ...
            case 'sqty':
            req.check('sqty', '104').isNumeric; /* invalid sqty */
            errors = req.validationErrors();
            if (errors) {
                errmsg = lcsAp.getMsgI18N(errors[0].msg);
                return errmsg;
