...
function lcsUI(nm) {
     ...
     * @return {Object}
     */
    lcsUI.prototype.config = function(param) {

        if (param === null || param === 'undefined') {
            lcsAp.log('invalid params:" lcsui.config');
            return false;
        } else if (param.map) {
            map = JSON.parse(require('fs').readFileSync(param.map));
            for (key in map) {
        ...
        auth.get(req.session.id, 
                 function(err, sess) {
                     if(sess && sess.views) {
                         ...
                         /* フィーチャーフォン判別 */
                         if (UserAgentUtil.isMobile(ua)) {
                             /* フィーチャーフォン用HTML返却*/
                             posts.layout = "layout3.ejs";
                             ...
                             res.render('scr/scr800', posts);
                             /*
                             res.render('scr/scr800', {}, function(err, posts) {
    ...
    lcsUI.prototype.checkUser = function(req, res) {
        ...
        var sql ='select userid,password,permit from user where userid=?';
