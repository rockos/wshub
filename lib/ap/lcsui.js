        ...
        auth.get(req.session.id, 
                 function(err, sess) {
                     if(sess && sess.views) {
                         ...
                         if (lcsUserAgentUtil.getOs(ua).indexOf('Android') >= 0 ||
                             lcsUserAgentUtil.getOs(ua).indexOf('iOS') >= 0) { /* モバイル */
                             res.render('scr/mob802', posts);
                         } else {   /* PC */
                         ...
                         } else if (lcsUserAgentUtil.getOs(ua).indexOf('Android') >= 0 ||
                                    lcsUserAgentUtil.getOs(ua).indexOf('iOS') >= 0) { /* モバイル */
                             posts.frameNavi = _frameInf[lcsAp.getLangI18N()];
                             posts.pageNavi = {};
                             res.render('scr/mob801', posts);
 ...
 */
    lcsUI.prototype.shoError= function(args, emsg){
     ...
     * @param {Object} emsg contents of error.
     */
    lcsUI.prototype.showError= function(args, emsg){
        if (args.req.body.clientID && args.req.body.nameSpace &&
            args.req.body.clientID.length && args.req.body.nameSpace.length) {
            var _id = args.req.body.clientID,
            _ns = args.req.body.nameSpace;
            /*request cancel*/
            args.res.send(204);
            if (typeof emsg === 'object') {
                lcsSOCK.io().of(_ns).sockets[_id].emit("errmsg",{
                        "mesg" : emsg.text,
                            "color" : emsg.warn});
            } else if (typeof emsg === 'number') {
                var msgobj = lcsAp.getMsgI18N(emsg);
                lcsSOCK.io().of(_ns).sockets[_id].emit("errmsg",{
                        "mesg" : msgobj.text,
                            "color" : msgobj.warn});
            } else if (typeof emsg === 'string') {
                lcsSOCK.io().of(_ns).sockets[_id].emit("errmsg",{
                        "mesg" : emsg,
                            "color" : ""});
            }
            ...
            if (typeof emsg === 'object') {
                args.errors = emsg;
                args.posts.mesg = emsg.text;
            } else {
                var msgobj = lcsAp.getMsgI18N(emsg);
                args.errors = msgobj;
                args.posts.mesg = msgobj.text;
            }
            var ua = args.req.headers['user-agent'];
            if (lcsUserAgentUtil.getOs(ua).indexOf('Android') >= 0 ||
                lcsUserAgentUtil.getOs(ua).indexOf('iOS') >= 0) { /* モバイル */
                args.res.render('scr/mob_error', args.posts);
            } else {   /* PC */
